...
import (
	dpfm_api_caller "data-platform-api-delivery-document-reads-rmq-kube/DPFM_API_Caller"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncDeliveryDocumentReads(accepter, &input, &output, l)
	if len(errs) != 0 {
...
func getAccepter(input *dpfm_api_input_reader.SDC) []string {
	...
	if accepter[0] == "All" {
		accepter = []string{
			"Header", "Headers", "Item", "Items", "ItemPicking", "ItemPickings", "Partner", "Address", "DeliverFromItems", "DeliverToItems",
		}
	}
	return accepter
}
