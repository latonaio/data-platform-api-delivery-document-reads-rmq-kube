...
func ConvertToItemPicking(rows *sql.Rows) (*[]ItemPicking, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&pm.DeliverToPlantKanbanContainer,
			&pm.DeliverFromParty,
			&pm.DeliverFromPlant,
			&pm.DeliverFromPlantStorageLocation,
			&pm.DeliverFromPlantStorageBin,
			&pm.DeliverFromPlantKanbanContainer,
			...
			&pm.ExternalReferenceDocument,
			&pm.ExternalReferenceDocumentItem,
			&pm.ExternalReferenceDocumentItemPickingID,
			&pm.CreationDate,
			&pm.CreationTime,
			&pm.LastChangeDate,
			&pm.LastChangeTime,
		...
		itemPicking = append(itemPicking, ItemPicking{
			...
			//DeliverToKanbanContainer:               			data.DeliverToKanbanContainer,
			DeliverFromParty:                data.DeliverFromParty,
			DeliverFromPlant:                data.DeliverFromPlant,
			DeliverFromPlantStorageLocation: data.DeliverFromPlantStorageLocation,
			DeliverFromPlantStorageBin:      data.DeliverFromPlantStorageBin,
			//DeliverFromKanbanContainer:               			data.DeliverFromKanbanContainer,
